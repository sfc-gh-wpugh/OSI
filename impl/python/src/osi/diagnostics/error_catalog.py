...
_EXPLANATIONS: dict[ErrorCode, str] = {
    ...
    ErrorCode.E_UNSAFE_REAGGREGATION: (
        ...
        "``PERCENTILE_CONT``) over a §6.7 chasm pre-aggregation or a "
        "§6.8.2 stitch. The §6.8.1 bridge plan is **conceptually** not in "
        "this family — D-027 describes it as a single-pass aggregate over "
        "the de-duplicated ``(measure-home-row, group-key)`` row set — so "
        "every aggregate category is well-defined there in principle. "
        "This reference implementation currently realises that route only "
        "for the distributive operators (``SUM``, ``COUNT``, ``MIN``, "
        "``MAX``) plus ``COUNT(DISTINCT)``; ``AVG``, ``MEDIAN``, and "
        "``PERCENTILE_CONT`` over an N:N bridge are still pending and "
        "surface this error today (tracked by "
        "``compliance/foundation-v0.1/tests/bridge/hard/t-016`` and "
        "``t-051``). Fixes: (a) switch to a distributive aggregate, "
        "(b) restate at a coarser grain that does not require chasm "
        "pre-aggregation, or (c) for M:N references, rely on the bridge "
        "plan that the engine already uses for distributive aggregates. "
        "(Spec: D-022 / D-027.)"
    ),
